...
public class ProductContent {
  ...
  public static ProductContent create(Product product, Random random) throws IOException {
    ...
    pc.name = product.getName();
    pc.description = product.getDescription();
    pc.productType = product.getProductType();
    pc.sceneSize = product.getSceneRasterSize();
    pc.startTime = product.getStartTime();
    pc.endTime = product.getEndTime();
    pc.geoLocations = create(product.getSceneGeoCoding(), product.getSceneRasterSize(), random);
    pc.rasters = create(product.getRasterDataNodes(), random);
    pc.vectorData = create(product.getVectorDataGroup());
    pc.metadata = create(product.getMetadataRoot(), random);
    pc.sampleCoding = create(product.getFlagCodingGroup(), product.getIndexCodingGroup());
    return pc;
  }
  ...
  private static String createPath(MetadataElement element, MetadataAttribute attributeAt) {
    ...
    } while (!element.getParentElement().getName().equals("metadata"));
  ...
  private static Raster create(RasterDataNode rdn, Random random) throws IOException {
    ...
    raster.setGeoLocations(create(rdn.getGeoCoding(), rdn.getRasterSize(), random));
    return raster;
  }
